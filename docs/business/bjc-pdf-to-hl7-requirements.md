...
>
> **Note (26 Aug 2026):** BJC has since asked for a two-folder model again — `Linked` plus an **`Unlinked`** folder that receives every email the converter assesses but does not file (in effect the Review folder under a new name, without Outlook categories). Rollout dates for the remaining three per-location fax mailboxes: `gofax.cht@` 1 Sep 2026, `gofax.bon@` and `gofax.bow@` 8 Sep 2026; the email mailboxes (stage 2) are deferred by BJC for a few months. See `docs/business/emails/2026-08-26-nicole-rollout-next-steps.md`.
